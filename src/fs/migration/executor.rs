...
pub fn process_migration(migration: &FsEntryMigration) -> Result<(), AppError> {
...
pub fn build_migration_source_path(migration: &FsEntryMigration) -> PathBuf {
    PathBuf::from(&migration.source_arg)
        .join(&migration.source_relative_path)
        .join(&migration.source_file_name)
        .clean()
}

pub fn build_migration_destination_path(migration: &FsEntryMigration) -> PathBuf {
    PathBuf::from(&migration.destination_arg)
